-- Manual rollback only: stop ALL writers and old/new API/UI traffic first.
-- Never register this file as a forward migration. Run with psql -X -v ON_ERROR_STOP=1.
BEGIN;
SET LOCAL lock_timeout = '5s';
LOCK TABLE public.employees, public.assets, public.assignments, public.agreements, public.assignment_checklists, public.audit_log IN ACCESS EXCLUSIVE MODE;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM public.assignments s WHERE s.assignment_kind='administrative'
   OR NOT EXISTS(SELECT 1 FROM public.agreements g WHERE g.assignment_id=s.id))
 OR EXISTS(SELECT 1 FROM public.audit_log WHERE entity='assignments' AND
   (details->>'assignment_kind'='administrative' OR details->>'create_agreement'='false')) THEN
   RAISE EXCEPTION 'ROLLBACK_REFUSED: administrative or agreement-free history exists; preserve data and forward-fix';
 END IF;
END $$;
DROP TRIGGER validate_assignment_policy ON public.assignments;
DROP TRIGGER validate_employee_archive ON public.employees;
DROP FUNCTION public.validate_assignment_policy();
DROP FUNCTION public.validate_employee_archive();
DROP VIEW public.assignments_list;
DROP FUNCTION public.create_assignment_complete(uuid,uuid,uuid,timestamptz,text,text,uuid,text,jsonb,text[],boolean,text);
ALTER TABLE public.assignments DROP COLUMN assignment_kind;
...
GRANT SELECT ON public.assignments_list TO authenticated;
COMMIT;
-- Migration ledger intentionally untouched: reconcile with the actual deployment owner/runner.
